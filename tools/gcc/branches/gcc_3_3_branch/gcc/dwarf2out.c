...
#ifdef __CYGWIN32__
static void fixpath(const char *orig, char *new);
static void
fixpath(orig, new)
     const char *orig;
     char *new;
{
  char *foo;
  cygwin32_conv_to_win32_path(orig, new);
  foo = new;
  while(*foo){
    if(*foo == '\\')
      *foo = '/';
     ...
     const char *filename;
{
  ...
  {
    char tmp[1024];
    fixpath(filename, tmp);
    add_name_attribute (die, tmp);
  }
#else
...
#ifdef __CYGWIN32__
  {
    char tmp[1024];
    fixpath(wd, tmp);
    add_AT_string (die, DW_AT_comp_dir, tmp);
  }
#else
