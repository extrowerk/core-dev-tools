...
static int remap, dump_map;
static char *generate_remap_file;
static struct file_name_map *found_mappings;
...
static char *remap_include_file PROTO((char *, struct file_name_list *, struct file_name_map **));
static int lookup_ino_include PROTO((struct include_file *));
static struct file_name_map * update_name_map PROTO((char *dirname, char *from, char *to));
static void dump_name_maps();
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++) {
	...
	if (!strcmp (argv[i], "-M"))
	  ...
	  print_deps = 1;
	/* For -MD and -MMD and -MMR options, write deps on file named by next arg.  */
	if (!strcmp (argv[i], "-MMR")) {
	  char *map_file;
	  if (i + 1 == argc)
	    fatal ("Filename missing after %s option", argv[i]);
	  i++;
	  generate_remap_file = argv[i];
	  map_file = strrchr( generate_remap_file, '/' );
	  if ( map_file )
	  	*map_file = '\0';
	  found_mappings = read_name_map( generate_remap_file );
	  if ( map_file )
	  	*map_file = '/';
	} else if (!strcmp (argv[i], "-MD")
	    || !strcmp (argv[i], "-MMD")) {
  ...
  for (i = 1; i < argc; i++)
    if (pend_files[i]) {
      struct include_file *inc;
      int fd = open_include_file (pend_files[i],pend_files[i], NULL_PTR, NULL_PTR, &inc);
      if (fd < 0) {
  ...
  for (i = 1; i < argc; i++)
    if (pend_includes[i]) {
      struct include_file *inc;
      int fd = open_include_file (pend_includes[i], pend_includes[i], NULL_PTR, NULL_PTR, &inc);
      if (fd < 0) {
...
get_filename:
  ...
  *fend = 0;
  ...
  if (absolute_filename (fbeg)) {
    strcpy (fname, fbeg);
    f = open_include_file (fbeg, fname, NULL_PTR, importing, &inc);
  } else {
      ...
      else
	{
      ...
      strcat (fname, fbeg);
#endif /* VMS */
      f = open_include_file (fbeg, fname, searchptr, importing, &inc);
      if (f != -1) {
	...
	break;
#else
/* With Cygwin, we can get ENXIO and ENAMETOOLONG, so check for them. 
   GP July, 2005 QNX */
#ifdef __CYGWIN__
      if (errno != ENOENT && errno != ENOTDIR && errno != ENXIO && errno != ENAMETOOLONG)
#else
...
void dump_name_maps()
{
    struct file_name_list *i;
    struct file_name_map *m;
    FILE *fp;
    ...
    fp = fopen( generate_remap_file, "w" );
    if ( fp == NULL )
	    return;

    for (m = found_mappings; m != NULL; m = m->map_next ) {
	fprintf( fp, "%s %s\n", m->map_from, m->map_to );
    }
    fclose(fp);
}
...
static struct file_name_map * update_name_map (char *dirname, char *from, char *to)
{
  struct file_name_map *ptr;
  size_t tolen, dirlen;

  for (ptr = found_mappings; ptr != NULL; ptr = ptr->map_next)
    {
	if (! strcmp (ptr->map_from, from))
	  return NULL;
    }
  tolen = strlen(to);
  ...
  ptr->map_from = strdup (from);

  /* Make the real filename absolute.  */
  if (absolute_filename (to))
	ptr->map_to = strdup (to);
  else
	{
...
static int
open_include_file (original_fname, filename, searchptr, importing, pinc)
     char *original_fname;
     char *filename;
     struct file_name_list *searchptr;
     U_CHAR *importing;
...
{
  struct file_name_map *map;
  char *fname = remap ? remap_include_file (filename, searchptr, &map) : filename;
  int fd = -2;
  ...
  *pinc = inc;
	if ( remap ) {
	  update_name_map( searchptr->fname, original_fname, fname );
	  dump_map = (map == NULL);
	}
  return fd;
}
...
static char *
remap_include_file (filename, searchptr, remapped)
     char *filename;
     struct file_name_list *searchptr;
	 struct file_name_map **remapped;
{
...
#ifdef __CYGWIN__
	/* for some reason, we sometimes get duplicate inode numbers
	 * under Cygwin - for now - check the size as well... 
	 */
	&& inc->st.st_size == i->st.st_size
#endif
     ...
     int vaxc_include;
{
  ...
  if (vaxc_include && !strchr (basename,'.'))
    strcat (basename, ".h");
