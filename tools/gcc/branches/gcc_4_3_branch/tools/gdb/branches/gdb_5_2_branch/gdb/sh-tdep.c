...
/* $MJC
 * Skip stores of arguments passed in r4-r7 to offsets off fp
 * Assume stores are done in order starting at r4 this probably only valid for debug code
 * This problem should be solved by having prologue length in the debug info.
 */
static unsigned short  const stRegs[4] = {
     0x2e42,    /* mov.l r4, @r14 */
     0x1e51,    /* mov.l r5, @(4,r14) */
     0x1e62,    /* mov.l r6, @(8,r14) */
     0x1e73     /* mov.l r7, @(12,r14) */
 };
static CORE_ADDR sh_skip_parmstr ( CORE_ADDR start_pc )
{
    int i;
        for( i = 0; i < 4; ++i ){
                unsigned short ins;
                ins = read_memory_integer (start_pc, 2);
        if( ins != stRegs[i] )break;
        start_pc += 2;
        }
    return start_pc;
}
...
sh_skip_prologue_hard_way (CORE_ADDR start_pc)
{
  ...
  start_pc = sh_skip_parmstr ( start_pc );
  return start_pc;
}
...
sh_skip_prologue (CORE_ADDR pc)
{
  ...
  if (post_prologue_pc != 0)
    {
      post_prologue_pc = sh_skip_parmstr ( post_prologue_pc );
...
sh_frame_chain (struct frame_info *frame)
{
  ...
  if (frame->pc && !inside_entry_file (frame->pc))
    return frame->extra_info->prev_fp ?: read_memory_integer (FRAME_FP (frame) + frame->extra_info->f_offset, 4);
  else
    return 0;
}
...
sh_find_callers_reg (struct frame_info *fi, int regnum)
{
  for (; fi; fi = fi->next)
	...
	if (!fi->pc)
	  return 0;
	if (regnum == SP_REGNUM && fi->extra_info->prev_sp )
		return fi->extra_info->prev_sp;
	if (regnum == FP_REGNUM && fi->extra_info->prev_fp )
		return fi->extra_info->prev_fp;
...
sh_nofp_frame_init_saved_regs (struct frame_info *fi)
{
  ...
  int rn;
  int have_fp = 0;
  int set_fp = 0;  /* The DEPTH at which we set fp */
  int have_set_fp = 0; /* Have we ACTUALLY set fp yet? */
  int set_fp_pc = 0; /* The PC at which fp is set */
  int depth;
  int current_depth; /* The REAL depth we are at - ie how much the SP has changed from ning of the function */
  int pc;
  int opc;
  int insn;
  int r1_val = 0;
  ...
  fi->extra_info->prev_fp = 0;
  fi->extra_info->prev_sp = 0;
  ...
  for (opc = pc + (2 * 28); pc < fi->pc && pc < opc; pc += 2)
    {
	...
	if ( !have_set_fp )
	  {
	    set_fp = depth; /* record the stack depth at which we actually set the FP */
	    set_fp_pc = pc;
	    if ( pc < fi->pc )
	      {
	    	have_set_fp = 1; /* only save that have done it if our pc is past it already */
	      }
	  }
	break;
     }
	...
	break;
      ...
      if ( pc < fi->pc )
	{
	  current_depth = depth; /* maintain a note of where we ACTUALLY are */
	}
    }

    if ( !have_set_fp )
      {
	/* if we haven't actually set the FP yet, we have to
	 * deduce it from the current SP
	 */
	fi->frame = (read_sp() + current_depth) - set_fp;
      }
  ...
  for (rn = 0; rn < NUM_REGS + NUM_PSEUDO_REGS; rn++)
    {
      if (where[rn] >= 0)
	{
	  /* Have we ACTUALLY saved it already? */
	  if ( where[rn] < current_depth )
	    {
  ...
  if (have_fp)
    {
      fi->extra_info->prev_sp = fi->frame + depth;
    }
  else
    {
...
/* 
 *
 * The following changes are intenteded to handle the (admittedly rare)
 * case where the program has stopped in code before the completion
 * of the prologue.
 *
 * This appears to be possible with the QNX 6.2.1 version of gcc 2.95.3
 * which can on occasion insert the code from the body of the function
 * before the completion of the prologue.  This confuses the hell out
...
sh_fp_frame_init_saved_regs (struct frame_info *fi)
{
  ...
  fi->extra_info->prev_fp = 0; /* keep a few more things around in extra info */
  fi->extra_info->prev_sp = 0;
