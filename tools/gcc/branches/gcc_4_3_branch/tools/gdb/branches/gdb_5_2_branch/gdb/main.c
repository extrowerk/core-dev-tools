...
extern int gdbtk_test (char *);
  ...
  if (execarg)
    {
      static char execbuf[PATH_MAX];
      cygwin_conv_to_posix_path(execarg, execbuf);
      execarg=execbuf;
    }
  if (symarg)
    {
      static char symbuf[PATH_MAX];
      cygwin_conv_to_posix_path(symarg, symbuf);
      symarg=symbuf;
    }
  if (corearg)
    {
      static char corebuf[PATH_MAX];
      cygwin_conv_to_posix_path(corearg, corebuf);
      corearg=corebuf;
    }
#endif
