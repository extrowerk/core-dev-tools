...
EOF
if [ "x${host}" = "x${target}" -a "${target_alias}" = "${TOOL_DIR}" ] ; then
  case " ${EMULATION_LIBPATH} " in
  *" ${EMULATION_NAME} "*)
cat >>e${EMULATION_NAME}.c <<EOF
	  const char *lib_path;
...
#define OPTION_SMALL_STUBS		(OPTION_EH_FRAME_HDR + 1)
...
cat >>e${EMULATION_NAME}.c <<EOF
    ...
    /* FIXME - This option should be phased out in place of -z now. */
    case OPTION_SMALL_STUBS:
...
cat >>e${EMULATION_NAME}.c <<EOF
  ...
  fprintf (file, _("  --small-stubs\t\tSame as -z now.  Only creates small stubs if supported\n"));
