  ...
  if (frame_reg_rtx != sp_reg_rtx)
    {
      ...
      if (sp_offset != 0)
		...
		emit_insn (gen_add3_insn (dest_reg, frame_reg_rtx, GEN_INT (sp_offset)));
		}
      else if (!savres)
...
rs6000_emit_prologue (void)
{
  ...
  if (!WORLD_SAVE_P (info)
  ...
  else if (!WORLD_SAVE_P (info) && !saving_GPRs_inline)
    {
      ...
      if (info->first_fp_reg_save != 64)
        {
		  rtx dest_reg = gen_rtx_REG (reg_mode, DEFAULT_ABI == ABI_AIX ? 12 : 11);
		  rtx offset = GEN_INT (sp_offset + (-8 * (64-info->first_fp_reg_save)));
      ...
      par = rs6000_make_savres_rtx (info, frame_reg_rtx,
				    ...
				    reg_mode,
				    /*savep=*/true, /*gpr=*/true,
					/*lr=*/(strategy & SAVRES_NOINLINE_GPRS_SAVES_LR) != 0);
