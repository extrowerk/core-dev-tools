...
/* Return a copy of the tree specified by NODE.
   See the implementation of build_new_1 (cp/init.c)
   Added by mikhailk */
     ...
     tree node;
{
	...
	if ( node == NULL_TREE ||
	     node == integer_zero_node ||
	     node == integer_one_node ||
	     node == size_zero_node ||
	     node == size_one_node ||
	     node == null_pointer_node ||
	     node == error_mark_node ||
	     node == void_type_node ||
	     node == integer_type_node ||
	     node == unsigned_type_node ||
	     node == char_type_node )
		return node;

	code = TREE_CODE( node );
	type = TREE_CODE_CLASS( code );
	t = copy_node( node );
	length = tree_code_length[code];

	TREE_CHAIN( t ) = copy_tree( TREE_CHAIN( node ) );
	...
	switch( type )
	{
		case 'c':  /* a constant */
      		if ( code == COMPLEX_CST )
      		{
				TREE_REALPART( t ) = copy_node( TREE_REALPART( node ) );
				TREE_IMAGPART( t ) = copy_node( TREE_IMAGPART( node ) );
			}
      		break;
		...
		case 'x':  /* something random, like an identifier.  */
			if ( code == TREE_LIST )
			{
				TREE_PURPOSE( t ) = copy_tree( TREE_PURPOSE( node ) );
				TREE_VALUE( t ) = copy_tree( TREE_VALUE( node ) );
			}
			else if ( code == TREE_VEC )
			{
				int i = 0;
				for ( ; i < TREE_VEC_LENGTH( node ); ++i )
					TREE_VEC_ELT( t, i ) = copy_tree( TREE_VEC_ELT( node, i ) );
			}
			break;
		...
		case 's':
		{
			int i = 0;
			for ( ; i < length; ++i )
				TREE_OPERAND( t, i ) = copy_tree( TREE_OPERAND( node, i ) );
			break;
		}
