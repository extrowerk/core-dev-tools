...
#define GCC_DRIVER_HOST_INITIALIZATION \
  do { \
    ...
    add_prefix (&exec_prefixes, concat (qnx_host, "/usr/ntoppc/bin/", NULL), \
...
#define CPP_PREDEFINES \
  "-DPPC -Dunix -D__QNXNTO__ -D__QNX__ -D__ELF__ -Asystem(unix) -Asystem(qnxnto) -Acpu(powerpc) -Amachine(powerpc) -D__PPC__"
...
-idirafter %$QNX_TARGET/usr/include %{EL:-D__LITTLEENDIAN__} %{!EL:-D__BIGENDIAN__}"
...
%{!shared: -u_start %$QNX_TARGET/ppc%{!EL:be}%{EL:le}/lib/%{pg:m}%{p:m}crt1.o }\
%$QNX_TARGET/ppc%{!EL:be}%{EL:le}/lib/crti.o \
%$QNX_TARGET/ppc%{!EL:be}%{EL:le}/lib/crtbegin.o"
...
#undef	LIB_SPEC
#define LIB_SPEC "\
-L %$QNX_TARGET/ppc%{EL:le}%{!EL:be}/lib/gcc/2.95.3 \
-L %$QNX_TARGET/ppc%{EL:le}%{!EL:be}/lib \
-L %$QNX_TARGET/ppc%{EL:le}%{!EL:be}/usr/lib \
-L %$QNX_TARGET/ppc%{EL:le}%{!EL:be}/opt/lib \
-rpath-link %$QNX_TARGET/ppc%{EL:le}%{!EL:be}/lib/gcc/2.95.3:\
%$QNX_TARGET/ppc%{EL:le}%{!EL:be}/lib:\
%$QNX_TARGET/ppc%{EL:le}%{!EL:be}/usr/lib:\
%$QNX_TARGET/ppc%{EL:le}%{!EL:be}/opt/lib \
...
#undef	ENDFILE_SPEC
#define ENDFILE_SPEC "\
%{!shared: %$QNX_TARGET/ppc%{!EL:be}%{EL:le}/lib/crtend.o \
