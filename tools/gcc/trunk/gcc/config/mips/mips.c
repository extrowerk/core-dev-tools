...
mips_classify_symbol (rtx x)
{
  if (GET_CODE (x) == LABEL_REF)
	...
	return SYMBOL_CONSTANT_POOL;
      if ((TARGET_ABICALLS || TARGET_ABICALLS) && !TARGET_ABSOLUTE_ABICALLS)
  ...
  if (TARGET_ABICALLS || TARGET_QNXPIC)
    {
...
mips_restore_gp (void)
{
  ...
  gcc_assert ((TARGET_ABICALLS || TARGET_QNXPIC) && TARGET_OLDABI);
  ...
  if (TARGET_QNXPIC) 
    {
      flag_pic = 1;
      if (mips_section_threshold > 0)
        warning (0, "%<-G%> is incompatible with %<-mqnx-pic%>");
  }
...
mips_conditional_register_usage (void)
{
  ...
  if (TARGET_QNXPIC)
    {
      int regno = QNXPIC_GOTREG_REGNUM;
      fixed_regs[regno] = call_used_regs[regno] = 1;
    } 
...
mips_save_reg_p (unsigned int regno)
{
  ...
  if (regno == 23 + GP_REG_FIRST && TARGET_QNXPIC) {
    return true;
  }
...
char *format_ptr;
	...
	if(GET_CODE(pattern) == PARALLEL || GET_CODE(pattern) == ADDR_VEC
...
char *fnname;
    ...
    /* We do the .cpload $31 insn for profiling qnxpic in
       mips_expand_prologue(), so don't do it again.  And we
       don't need it (sets up the GOT) if we're not pic.  */
    if(current_function_profile || !TARGET_QNXPIC)
      return;
     ...
     */
    if(is_sym_ref == PIC_GLOBAL_SYMBOL_REF) {
...
mips_expand_prologue (void)
{
  ...
  /* If we're profiling qnxpic code, emit the .cpload $31 here
     so the GOT is set up before _mcount tries to use it.  Otherwise
     let mips_finalize_pic() decide if we need it or not.  */
  if (TARGET_QNXPIC && !current_function_is_leaf)
    {
      rtx gp_offset=GEN_INT(0);
      emit_insn (gen_qnxpic_loadgp(gp_offset));
    }
...
mips_restore_reg (rtx reg, rtx mem)
{
  ...
  if (TARGET_QNXPIC && reg == QNXPIC_GOTREG_REGNUM)
    emit_insn(gen_blockage());
		      ...
		      tree function)
{
  ...
  if (TARGET_MIPS16 || TARGET_ABICALLS || TARGET_LONG_CALLS || TARGET_QNXPIC)
    {
