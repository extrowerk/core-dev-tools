...
static bfd *get_exec_bfd(void)
{
	return exec_bfd ? exec_bfd : symfile_objfile ? symfile_objfile->obfd : NULL;
}
...
look_for_base (int fd, CORE_ADDR baseaddr)
{
  ...
  if (fd == -1
      || (get_exec_bfd() != NULL
	  && fdmatch (fileno ((FILE *) (get_exec_bfd()->iostream)), fd)))
    {
      return (0);
    }
...
elf_locate_base (void)
{
  ...
  dyninfo_sect = bfd_get_section_by_name (get_exec_bfd(), ".dynamic");
  if (dyninfo_sect == NULL)
    return 0;
  dyninfo_addr = bfd_section_vma (get_exec_bfd(), dyninfo_sect);

  /* Read in .dynamic section, silently ignore errors.  */
  dyninfo_sect_size = bfd_section_size (get_exec_bfd(), dyninfo_sect);
  ...
  arch_size = bfd_get_arch_size (get_exec_bfd());
  ...
  if (arch_size == 32)
	   ...
	   buf += sizeof (Elf32_External_Dyn))
	{
	  ...
	  dyn_tag = bfd_h_get_32 (get_exec_bfd(), (bfd_byte *) x_dynp->d_tag);
	  if (dyn_tag == DT_NULL)
	    break;
	  else if (dyn_tag == DT_DEBUG)
	    {
	      dyn_ptr = bfd_h_get_32 (get_exec_bfd(), 
	   ...
	   buf += sizeof (Elf64_External_Dyn))
	{
	  ...
	  dyn_tag = bfd_h_get_64 (get_exec_bfd(), (bfd_byte *) x_dynp->d_tag);
	  if (dyn_tag == DT_NULL)
	    break;
	  else if (dyn_tag == DT_DEBUG)
	    {
	      dyn_ptr = bfd_h_get_64 (get_exec_bfd(), 
				      (bfd_byte *) x_dynp->d_un.d_ptr);
	      return dyn_ptr;
	    }
	}
...
locate_base (void)
{
  ...
  if (debug_base == 0)
    {
      if (get_exec_bfd() != NULL
	  && bfd_get_flavour (get_exec_bfd()) == bfd_target_elf_flavour)
...
enable_break (void)
{
  ...
  interp_sect = bfd_get_section_by_name (get_exec_bfd(), ".interp");
  if (interp_sect)
    {
      ...
      interp_sect_size = bfd_section_size (get_exec_bfd(), interp_sect);
      buf = alloca (interp_sect_size);
      bfd_get_section_contents (get_exec_bfd(), interp_sect,
...
svr4_relocate_main_executable (void)
{
  ...
  interp_sect = bfd_get_section_by_name (get_exec_bfd(), ".interp");
  if (interp_sect == NULL 
      && (bfd_get_file_flags (get_exec_bfd()) & DYNAMIC) != 0
      && bfd_get_start_address (get_exec_bfd()) != pc)
    {
	 ...
	 ABI and is left unspecified in some of the earlier editions.  */
      ...
      displacement = pc - bfd_get_start_address (get_exec_bfd());
      changed = 0;
