...
/* Even though the tx79 is a MIPS III chip and uses the EABI, it only
...
#define REGISTER_VIRTUAL_TYPE(N) tx79_register_virtual_type(N)
struct type *tx79_register_virtual_type();

#undef FIRST_EMBED_REGNUM
#define FIRST_EMBED_REGNUM 73       /* First CP0 register for embedded use */
...
/* for tx79, add a bunch of 64-bit registers that are really the upper */
/* 64 bits of the 128-bit registers */

#define FIRST_COP0_REG 128
#undef NUM_REGS
#define NUM_REGS 150		/* tx79 */
...
#define TM_PRINT_INSN_MACH bfd_mach_mips7900

/* Define DO_REGISTERS_INFO() to do machine-specific formatting
   of register dumps. */

#undef DO_REGISTERS_INFO
#define DO_REGISTERS_INFO(_regnum, fp) tx79_do_registers_info(_regnum, fp)
extern void tx79_do_registers_info PARAMS ((int, int));
