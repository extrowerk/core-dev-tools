...
static struct breakpoint *find_breakpoint(CORE_ADDR placed_addr)
{
  struct breakpoint *pivot;
  for (pivot = breakpoint_chain; pivot; pivot = pivot->next)
  {
    if (pivot->loc->target_info.placed_address == placed_addr)
      return pivot;
  }
  return NULL;
}
...
void nto_fetch_registers (int regno);

static void nto_prepare_to_store ();

static void nto_store_registers (int regno);
...
static enum target_signal target_signal_from_nto (int sig);

static int target_signal_to_nto (enum target_signal sig);
...
fetch_regs (struct regcache *regcache, int regset, int supply)
{
  ...
  len = nto_register_area (-1, regset, &offset);
  if (len < 1)
    return 0;
...
nto_fetch_registers (int regno)
{
  ...
  nto_trace (0) ("nto_fetch_registers(regno %d)\n", regno);
  ...
  if (regno == -1)		/* Send them all.  */
    {
      for (regset = NTO_REG_GENERAL; regset < NTO_REG_END; regset++)
	{
	  len = nto_register_area (-1, regset, &off);
	  if (len < 1)
	    continue;
...
{
  pid_t cur_pid, start_tid = 1, total_tids = 0;
