...
static void nto_prepare_to_store (struct target_ops *,
...
static void nto_open (const char *name, int from_tty);
static void nto_close (struct target_ops *);
...
static void nto_detach (struct target_ops *ops, const char *args, int from_tty);
...
static int nto_insert_hw_breakpoint (struct target_ops *ops, struct gdbarch *,
				     struct bp_target_info *);

static int nto_remove_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr,
				     int len, enum target_hw_bp_type type,
				     struct expression *);

static int nto_insert_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr,
				     int len, enum target_hw_bp_type type,
				     struct expression *);
...
nto_get_thread_alive (struct target_ops *ops, ptid_t th)
{
  ...
  if (recv.pkt.hdr.cmd == DSrMsg_okdata)
	...
	If it is not, then requested thread is dead.  */
      ...
      if (!ptidinfo->state)
	{
	  return nto_get_thread_alive (ops, ptid_build (ptid_get_pid (th), 0,
				       returned_tid+1));
	}
    }
  else if (recv.pkt.hdr.cmd == DSrMsg_okstatus)
...
static void
nto_close (struct target_ops *ops)
{
...
static void
nto_open (const char *name, int from_tty)
{
  int tries = 0;
  void (*ofunc) (int);
...
static void
nto_attach (struct target_ops *ops, const char *args, int from_tty)
{
  const enum bfd_endian byte_order = gdbarch_byte_order (target_gdbarch ());
  ptid_t ptid;
  struct inferior *inf;
  ...
  inferior_ptid = ptid_build (EXTRACT_SIGNED_INTEGER (&recv->pkt.notify._32.pid, 4,
						      byte_order),
			      0,
 ...
 /* NYI: add symbol information for process.  */
  ...
  recv->pkt.notify._32.pid = ptid_get_pid (ptid);
  recv->pkt.notify._32.tid = ptid_get_tid (ptid);
...
static void
nto_detach (struct target_ops *ops, const char *args, int from_tty)
{
  DScomm_t tran, recv;
  const enum bfd_endian byte_order = gdbarch_byte_order (target_gdbarch ());
  struct inferior *inf;
  ...
  tran.pkt.detach.pid = ptid_get_pid (inferior_ptid);
...
static void
nto_prepare_to_store (struct target_ops *ops, struct regcache *regcache)
{
...
static int
nto_insert_breakpoint (struct target_ops *ops, CORE_ADDR addr,
...
static int
nto_to_insert_breakpoint (struct target_ops *ops, struct gdbarch *gdbarch,
...
static int
nto_remove_breakpoint (struct target_ops *ops, CORE_ADDR addr,
		       gdb_byte *contents_cache)
{
  const enum bfd_endian byte_order = gdbarch_byte_order (target_gdbarch ());
  DScomm_t tran, recv;
...
static int
nto_to_remove_breakpoint (struct target_ops *ops, struct gdbarch *gdbarch,
			  struct bp_target_info *bp_tg_inf,
			  enum remove_bp_reason reason)
{
  ...
  return nto_remove_breakpoint (ops, bp_tg_inf->placed_address,
				bp_tg_inf->shadow_contents);
}

static int
nto_remove_hw_breakpoint (struct target_ops *ops, struct gdbarch *gdbarch,
			  struct bp_target_info *bp_tg_inf)
{
  nto_trace (0) ("%s ( bp_tg_inf=%p )\n", __func__, bp_tg_inf);
...
static int
nto_can_run (struct target_ops *const ops)
{
  nto_trace (0) ("%s ()\n", __func__);
  return 0;
}
...
static int
nto_can_use_hw_breakpoint (struct target_ops *ops, enum bptype type, int cnt,
			   int othertype)
{
  return 1;
}
...
static int
nto_insert_fork_catchpoint (struct target_ops *ops, int pid)
{
...
static int
nto_remove_fork_catchpoint (struct target_ops *ops, int pid)
{
  nto_trace (0) ("%s", __func__);
  return 0;
}
...
static int
nto_insert_vfork_catchpoint (struct target_ops *ops, int pid)
{
  nto_trace (0) ("%s", __func__);
  return 0;
}
...
static int
nto_remove_vfork_catchpoint (struct target_ops *ops, int pid)
{
  nto_trace (0) ("%s", __func__);
  return 0;
}
...
static int
nto_insert_exec_catchpoint (struct target_ops *ops, int pid)
{
  nto_trace (0) ("%s", __func__);
  return 0;
}
...
static int
nto_remove_exec_catchpoint (struct target_ops *ops, int pid)
{
  nto_trace (0) ("%s", __func__);
  return 0;
}
...
static int
nto_follow_fork (struct target_ops *ops, int follow_child, int detach_fork)
{
  ...
  gdb_assert (parent_inf->pid == ptid_get_pid (inferior_ptid));
...
static int
nto_supports_multi_process (struct target_ops *ops)
{
  return 1;
}
...
static void
nto_generic_load (struct target_ops *ops, const char *args, int from_tty)
{
  generic_load (args, from_tty);
}

static int
nto_can_async_p (struct target_ops *ops)
{
  /* Not yet. */
  return 0;
}
...
static int
nto_supports_non_stop (struct target_ops *ops)
{
  /* Not yet. */
  return 0;
}
...
/* Terminal handling.  */
static void
nto_terminal_init (struct target_ops *const ops)
{
}

static void
nto_terminal_inferior (struct target_ops *const ops)
{
}

static void
nto_terminal_ours_for_output (struct target_ops *const ops)
{
}

static void
nto_terminal_ours (struct target_ops *const ops)
{
}

struct void
nto_terminal_save_ours (struct target_ops *const ops)
{
}

struct void
nto_terminal_info (struct target_ops *const ops, const char *arg, int c)
{
}
...
or `pty' to launch `pdebug' for debugging.";
  ...
  nto_ops.to_remove_hw_breakpoint = nto_remove_hw_breakpoint;
  ...
  nto_ops.to_load = nto_generic_load;
  ...
  nto_ops.to_update_thread_list = nto_find_new_threads;
  nto_ops.to_stop = 0;
  ...
  nto_ops.to_insert_fork_catchpoint = nto_insert_fork_catchpoint;
  nto_ops.to_remove_fork_catchpoint = nto_remove_fork_catchpoint;
  nto_ops.to_insert_vfork_catchpoint = nto_insert_vfork_catchpoint;
  nto_ops.to_remove_vfork_catchpoint = nto_remove_vfork_catchpoint;
  nto_ops.to_follow_fork = nto_follow_fork;
  nto_ops.to_insert_exec_catchpoint = nto_insert_exec_catchpoint;
  nto_ops.to_remove_exec_catchpoint = nto_remove_exec_catchpoint;
  nto_ops.to_supports_multi_process = nto_supports_multi_process;
  nto_ops.to_verify_memory = nto_verify_memory;
  nto_ops.to_can_async_p = nto_can_async_p;
  nto_ops.to_supports_non_stop = nto_supports_non_stop;

#if 0
  nto_ops.to_terminal_init = nto_terminal_init;
  nto_ops.to_terminal_inferior = nto_terminal_inferior;
  nto_ops.to_terminal_ours_for_output = nto_terminal_ours_for_output;
  nto_ops.to_terminal_save_ours = nto_terminal_save_ours;
  nto_ops.to_terminal_info = nto_terminal_info;
#endif
}
	  ...
	  if(ti)
	    {
	      ...
	      priv = (struct private_thread_info *)xrealloc(
		       ti->priv,
  ...
  do {
      ...
      if (tip->tid < 0)
	{
      ...
      if(new_thread && !new_thread->priv){
        new_thread->priv = XNEW(struct private_thread_info);
        new_thread->priv->name[0] = '\0';
      }
      if (new_thread && new_thread->priv)
	memcpy(new_thread->priv, tip, sizeof(*tip));
      total_tids++;
    }
...
static int
nto_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr,
		   int len, enum target_hw_bp_type type)
{
  DScomm_t tran, recv;
  const enum bfd_endian byte_order = gdbarch_byte_order (target_gdbarch ());
...
static int
nto_remove_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr, int len,
			  enum target_hw_bp_type type, struct expression *exp)
{
  return nto_hw_watchpoint (ops, addr, -1, type);
}

static int
nto_insert_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr, int len,
			  enum target_hw_bp_type type, struct expression *exp)
{
  return nto_hw_watchpoint (ops, addr, len, type);
}
