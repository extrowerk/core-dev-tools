...
static int nto_remove_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr,
				     int len, int type, struct expression *);
...
nto_detach (struct target_ops *ops, const char *args, int from_tty)
{
  ...
  if (args)
    {
      int sig = nto_gdb_signal_to_target (target_gdbarch (), atoi (args));
...
static int
nto_can_use_hw_breakpoint (struct target_ops *ops, int type, int cnt,
			   int othertype)
{
  return 1;
}
...
static void
nto_generic_load (struct target_ops *ops, char *args, int from_tty)
{
  ...
  do {
      ...
      if (tip->tid < 0)
	{
      ...
      if(new_thread && !new_thread->priv){
        new_thread->priv = xmalloc(sizeof(struct private_thread_info));
...
static int
nto_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr, int len, int type)
{
  DScomm_t tran, recv;
  const enum bfd_endian byte_order = gdbarch_byte_order (target_gdbarch ());
  unsigned subcmd;
...
nto_remove_hw_watchpoint (struct target_ops *ops, CORE_ADDR addr, int len,
			  int type, struct expression *exp)
{
