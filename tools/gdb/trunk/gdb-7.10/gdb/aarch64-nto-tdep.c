...
aarch64nto_supply_gregset (struct regcache *regcache, const gdb_byte *gregs)
{
  ...
  for (regno = AARCH64_X0_REGNUM; regno <= AARCH64_CPSR_REGNUM; regno++)
    regcache_raw_supply (regcache, regno, gregs + AARCH64_GREGSZ * regno);
}
...
aarch64nto_supply_fpregset (struct regcache *regcache, const gdb_byte *fpregs)
{
  ...
  for (regno = AARCH64_V0_REGNUM; regno <= AARCH64_V31_REGNUM; regno++)
    regcache_raw_supply (regcache, regno, fpregs + AARCH64_FPREGSZ
			...
			const int regset, gdb_byte *const data)
{
  if (regset == NTO_REG_GENERAL)
	...
	regcache_raw_collect (regcache, regno, data + AARCH64_GREGSZ * regno);
      return 0;
    }
  else if (regset == NTO_REG_FLOAT)
    {
...
static int
aarch64nto_sigtramp_p (struct frame_info *this_frame)
{
...
static CORE_ADDR
aarch64nto_sigcontext_addr (struct frame_info *this_frame)
{
...
extern initialize_file_ftype _initialize_aarch64nto_tdep;

void
_initialize_aarch64nto_tdep (void)
{
