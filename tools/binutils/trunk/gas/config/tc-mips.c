...
#elif TE_QNX
      return (target_big_endian
	      ? "elf32-qnxbigmips" : "elf32-qnxlittlemips");
#else
      return (target_big_endian
	      ? (HAVE_64BIT_OBJECTS
		 ? "elf64-bigmips"
...
#define OPTION_QNX_PIC           (OPTION_ELF_BASE + 12)
  {"qnx_pic", no_argument, NULL, OPTION_QNX_PIC},
  {"mqnx-pic", no_argument, NULL, OPTION_QNX_PIC},
#define OPTION_NO_QNX_PIC        (OPTION_ELF_BASE + 13)
...
mips_fix_adjustable (fixS *fixp)
{
  ...
  if ((mips_pic == QNX_PIC) &&
      ...
      || fixp->fx_r_type == BFD_RELOC_32))
    {
      ...
      if (S_IS_EXTERNAL(sym) || S_IS_WEAK(sym)) return 0;
    }

  /* If siymbol SYM is in a mergeable section, relocations of the form
     SYM + 0 can usually be made sectoin-relative.  The mergeable data
