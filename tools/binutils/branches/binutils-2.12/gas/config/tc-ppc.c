     ...
     char *arg;
{
  switch (c)
	...
	ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_ALTIVEC;
      /* QNX jbaker - re-added 'vec' as a valid argument for source compat.*/
      else if ((strcmp (arg, "altivec") == 0)||(strcmp(arg, "vec") == 0))
        {
          if (ppc_cpu == 0)
            ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_ALTIVEC;
          else
     ...
     int ignore ATTRIBUTE_UNUSED;
{
      ...
      *input_line_pointer = '\0';
      ...
      /* -mpwrx and -mpwr2 mean to assemble for the IBM POWER/2
         (RIOS2).  */
      if (strcmp (name, "pwrx") == 0 || strcmp (name, "pwr2") == 0)
	ppc_cpu = PPC_OPCODE_POWER | PPC_OPCODE_POWER2;
      /* -mpwr means to assemble for the IBM POWER (RIOS1).  */
	...
	ppc_cpu = PPC_OPCODE_PPC;
      /* -m403 and -m405 mean to assemble for the PowerPC 403/405.  */
      else if (strcmp (name, "403") == 0
               || strcmp (name, "405") == 0)
	ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_403;
      else if (strcmp (name, "7400") == 0
               || strcmp (name, "7410") == 0
               || strcmp (name, "7450") == 0
               || strcmp (name, "7455") == 0)
	ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_ALTIVEC;
      /* QNX jbaker - re-added 'vec' as a valid option for code compat. */ 
      else if ((strcmp (name, "altivec") == 0)||(strcmp(name, "vec") == 0))
        {
          if (ppc_cpu == 0)
            ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_ALTIVEC;
          else
      ...
      /* -mppc64 and -m620 mean to assemble for the 64-bit PowerPC
	 620.  */
      else if (strcmp (name, "ppc64") == 0 || strcmp (name, "620") == 0)
	{
	  ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_64;
	  ppc_size = PPC_OPCODE_64;
	}
      else if (strcmp (name, "ppc64bridge") == 0)
	{
	  ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_64_BRIDGE | PPC_OPCODE_64;
	  ppc_size = PPC_OPCODE_64;
	}
      /* -mbooke/-mbooke32 mean enable 32-bit BookE support.  */
      else if (strcmp (name, "booke") == 0 || strcmp (name, "booke32") == 0)
	ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_BOOKE;
      /* -mbooke64 means enable 64-bit BookE support.  */
      else if (strcmp (name, "booke64") == 0)
	{
	  ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_BOOKE | 
		    PPC_OPCODE_BOOKE64 | PPC_OPCODE_64;
	  ppc_size = PPC_OPCODE_64;
	}
      else if (strcmp (name, "power4") == 0)
	{
	  ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_64 | PPC_OPCODE_POWER4;
	  ppc_size = PPC_OPCODE_64;
	}
	...
	ppc_cpu = PPC_OPCODE_ANY;
      ...
      {
        as_bad (_("invalid assembly arg to pseudo opcode .set: %s"), name);
        return;
      }
