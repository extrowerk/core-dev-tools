...
#define SGI_COMPAT(abfd) \
  (IRIX_COMPAT (abfd) != ict_none || QNX_COMPAT (abfd) != qct_none)

/* Whether we are trying to be compatible with QNX at all.  */
#define QNX_COMPAT(abfd) \
  (get_elf_backend_data (abfd)->elf_backend_mips_qnx_compat (abfd))
    ...
    case R_MIPS_64:
      if ((info->shared
	   || (elf_hash_table (info)->dynamic_sections_created
	       && h != NULL
	       && h->root.def_dynamic
	       && (QNX_COMPAT(abfd) && h->root.needs_copy)
	       && !h->root.def_regular))
	  && r_symndx != 0
				    ...
				    bfd_vma *addendp, asection *input_section)
{
  ...
  if (IRIX_COMPAT (output_bfd) == ict_irix5 || QNX_COMPAT (output_bfd) == qct_qnx6)
    {
...
_bfd_mips_elf_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if ((IRIX_COMPAT (abfd) == ict_irix5 || IRIX_COMPAT (abfd) == ict_none || QNX_COMPAT (abfd) == qct_qnx6)
      && !info->shared
  ...
  if (IRIX_COMPAT (abfd) == ict_irix5 || QNX_COMPAT (abfd) == qct_qnx6)
    {
				     ...
				     struct elf_link_hash_entry *h)
{
  ...
  if (QNX_COMPAT(dynobj)) {
    if (h->got.offset == MINUS_ONE)
    {
	  mips_elf_record_global_got_symbol(h, dynobj, info, g, 0);
	  ++g->global_gotno;
	  s->size = (g->local_gotno
		    + g->global_gotno
		    + g->tls_gotno) * MIPS_ELF_GOT_SIZE (dynobj);
    }
  }
  ...
  BFD_ASSERT (dynobj != NULL
	      && (h->needs_plt
		  || (QNX_COMPAT(dynobj) && h->needs_copy)
		  || h->u.weakdef != NULL
		  || (h->def_dynamic
		      && h->ref_regular
		      && !h->def_regular)));
  ...
  if (! info->relocatable
	  ...
	  || !h->def_regular))
    {
      if (QNX_COMPAT(dynobj)) sreldyn = bfd_get_section_by_name (dynobj, ".rel.dyn");
				     ...
				     struct bfd_link_info *info)
{
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	return FALSE;
	 ...
	 (QNX_COMPAT(dynobj) && bfd_get_section_by_name (dynobj, ".rel.bss"))) 
	{
	...
	return FALSE;
      ...
      if ((IRIX_COMPAT (dynobj) == ict_irix5 || QNX_COMPAT (dynobj) == qct_qnx6)
	  && ! MIPS_ELF_ADD_DYNAMIC_ENTRY (info, DT_MIPS_HIPAGENO, 0))
	return FALSE;

      if ((IRIX_COMPAT (dynobj) == ict_irix6 || QNX_COMPAT (dynobj) == qct_qnx6)
  ...
  if (h->plt.offset != MINUS_ONE)
	...
	return FALSE;
	 ...
	 to its stub address when unlinking a shared object.  */
      if (QNX_COMPAT(output_bfd) && info->shared) {
 	 sym->st_value = 0;
      } else {
...
_bfd_mips_elf_additional_program_headers (bfd *abfd)
{
  ...
  if ((IRIX_COMPAT (abfd) == ict_irix6 || QNX_COMPAT (abfd) == qct_qnx6)
				  ...
				  struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  ...
  if (NEWABI_P (abfd)
	...
	if ((*pm)->p_type == PT_DYNAMIC)
	  break;
      m = *pm;
      if (m != NULL && IRIX_COMPAT (abfd) == ict_none && QNX_COMPAT (abfd) == qct_none)
	{
