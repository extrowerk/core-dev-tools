     ...
     flagword flags;
{
  switch (flags & EF_MIPS_MACH)
    ...
    case E_MIPS_MACH_7900:
      return bfd_mach_mips7900;
     ...
     boolean linker ATTRIBUTE_UNUSED;
{
  ...
  switch (bfd_get_mach (abfd))
    ...
    case bfd_mach_mips7900:
      val = E_MIPS_ARCH_3 | E_MIPS_MACH_7900;
      break;
     ...
     struct elf_link_hash_entry *h;
{
  ...
  if (! hmips->no_fn_stub
      ...
      if ((h->elf_link_hash_flags & ELF_LINK_HASH_DEF_REGULAR) == 0)
	{
	  ...
	  h->elf_link_hash_flags |= ELF_LINK_HASH_DEF_REGULAR;
	  if ( sreldyn != NULL ) {
	    sreldyn->_raw_size -= hmips->possibly_dynamic_relocs * sizeof (Elf32_External_Rel);
