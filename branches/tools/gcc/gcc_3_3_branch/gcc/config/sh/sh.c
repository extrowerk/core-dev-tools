     ...
     enum reg_class srcclass, dstclass;
{
  ...
  return 4;
   ...
   /* This is my interpretation of Joern Rennecke's comment in the
      gcc bugzilla report 11587:
     "The [original] patch tests the wrong condition.  A copy from T to a general
      purpose register is unproblematic and cheap.  The need for a secondary reload
      arises on the contrary when the other class is not a general purpose register.
      See how FPSCR_REGS is handled."
      So I copied the FPSCR case above, changed it to T_REGS, and gave it the weight
      from the original patch - 5.  GP QNX, Oct 25, 2004.  */

   if ((srcclass == T_REGS && ! REGCLASS_HAS_GENERAL_REG (dstclass))
       || (dstclass == T_REGS && ! REGCLASS_HAS_GENERAL_REG (srcclass)))
   return 5;
