...
(define_expand "divdi3"
  [(set (reg:DI 20) (match_operand:DI 1 "general_operand" ""))
   (set (reg:DI 21) (match_operand:DI 2 "general_operand" ""))
   (set (match_operand:DI 0 "register_operand" "") (match_dup 2))]
  ""
  "
{
    rtx x, sym;

    if ( TARGET_HARD_SH4 && !TARGET_FPU_SINGLE ) {
	sym = gen_rtx_SYMBOL_REF(Pmode, \"__divdi3_i4\");
    }
    else {
	sym = gen_rtx_SYMBOL_REF(Pmode, \"__divdi3\");
    }
    x = emit_library_call_value ( sym, NULL_RTX, 1, DImode, 2,
  ...
  "
{
    ...
    if ( TARGET_HARD_SH4 && !TARGET_FPU_SINGLE ) {
	sym = gen_rtx_SYMBOL_REF(Pmode, \"__udivdi3_i4\");
    }
    else {
	sym = gen_rtx_SYMBOL_REF(Pmode, \"__udivdi3\");
    }
    x = emit_library_call_value ( sym, NULL_RTX, 1, DImode, 2,
			   operands[1], DImode, operands[2], DImode );
    emit_move_insn (operands[0], x);
