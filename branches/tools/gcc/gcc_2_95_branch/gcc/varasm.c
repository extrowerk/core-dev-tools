...
#define MAX_OFILE_ALIGNMENT BIGGEST_ALIGNMENT
  ...
  if (maximum_field_alignment != 0)
    DECL_ALIGN(decl) = align = MIN (align, maximum_field_alignment);
  else
    DECL_ALIGN (decl) = align;

  if (TYPE_PACKED(TREE_TYPE(decl)))
    DECL_ALIGN(decl) = align = MAX (BITS_PER_UNIT, maximum_field_alignment);
  ...
  if (align > BITS_PER_UNIT || maximum_field_alignment != 0 || TYPE_PACKED(TREE_TYPE(decl)) )
