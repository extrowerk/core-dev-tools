...
static boolean elfcore_grok_nto_gregs PARAMS ((bfd *, Elf_Internal_Note *, pid_t));
static boolean elfcore_grok_nto_status PARAMS ((bfd *, Elf_Internal_Note *, pid_t *));
static boolean elfcore_grok_nto_note PARAMS ((bfd *, Elf_Internal_Note *));
     ...
     int index;
{
  ...
  switch (hdr->p_type)
    {
    case PT_NULL:
      ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "null");
      break;
    case PT_LOAD:
      ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "load");
      break;

    case PT_DYNAMIC:
      ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "dynamic");
      break;

    case PT_INTERP:
      ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "interp");
      break;

    case PT_NOTE:
      if (! _bfd_elf_make_section_from_phdr (abfd, hdr, index, "note"))
	ret = false;
      else if (! elfcore_read_notes (abfd, (file_ptr) hdr->p_offset, hdr->p_filesz))
	ret = false;
      ret = true;
      break;

    case PT_SHLIB:
      ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "shlib");
      break;

    case PT_PHDR:
      ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "phdr");
      break;
	...
	ret = (*bed->elf_backend_section_from_phdr) (abfd, hdr, index);
      else
	ret = _bfd_elf_make_section_from_phdr (abfd, hdr, index, "segment");
      break;
    }
    return(ret);
}
...
#if defined(HAVE_SYS_PROCFS_H) && !defined(__QNXTARGET__)
