...
make_cv_type (int cnst, int voltl, struct type *type, struct type **typeptr)
{
  ...
  while (ntype != type)
	  ...
	  (TYPE_VOLATILE (ntype) == voltl))
	{
      ...
      /* If this case is true, the list of types is corrupt.  The list should
         be circular but for some reason it winds up looping back to the
         child of type instead of type.  Break out and let it get fixed down
         below.  Ugly hack but all this code is gone/rewritten in new gdbs.  */
      if(TYPE_CV_TYPE(ntype) == TYPE_CV_TYPE(type))
	break;
    }
