...
struct {
  ...
  { "generic", mips_generic_reg_names, sizeof(mips_generic_reg_names) / sizeof(char *) },
  { "r3041", mips_r3041_reg_names, sizeof(mips_r3041_reg_names) / sizeof(char *) },
  { "r3051", mips_r3051_reg_names, sizeof(mips_r3051_reg_names) / sizeof(char *) },
  { "r3071", mips_r3081_reg_names, sizeof(mips_r3081_reg_names) / sizeof(char *) },
  { "r3081", mips_r3081_reg_names, sizeof(mips_r3081_reg_names) / sizeof(char *) },
  { "lsi33k", mips_lsi33k_reg_names, sizeof(mips_lsi33k_reg_names) / sizeof(char *) },
  { "tx79", mips_tx79_reg_names, sizeof(mips_tx79_reg_names) / sizeof(char *) },
  { NULL, NULL, 0 }
};
/* *INDENT-ON* */
...
#ifdef NUM_TX79_REGS /* Implies that NUM_REGS has been defined as this function */
int
mips_num_regs(void)
{
    int i;

    if(mips_processor_type)
	for (i = 0; mips_processor_type_table[i].name != NULL; ++i)
	    if (strcasecmp (mips_processor_type, mips_processor_type_table[i]. name) == 0)
		return(mips_processor_type_table[i].numregs);
    return(NUM_TX79_REGS);
}
#endif
...
mips_reg_size(void)
{
    /* The mipsbe (broadcom) board was being asked for 8 bytes of memory
       when doing a backtrace which was making it impossible to bt out of
       shared libs.  Best to return 4 and ponder tx79 support again when
       we go to gdb 6.0.  */
    return 4;

    if(mips_processor_type)
    {
	if(!strcasecmp("tx79", mips_processor_type))
	    return(8);
	else
	    return(4);
    }
...
int 
mips_gdb_target_is_mips64(void)
{
    if(mips_processor_type)
    {
	if(!strcasecmp("tx79", mips_processor_type))
...
int
mips_max_register_raw_size(void)
{
...
#ifdef NUM_TX79_REGS
static CORE_ADDR temp_saved_regs[NUM_TX79_REGS];
#else
...
mips_frame_chain (struct frame_info *frame)
{
  ...
  if (find_pc_partial_function ((frame)->pc, 0, &tmp, 0))
    if (tmp == entry_point_address ())
...
{
  /* If we have selected the tx79 128 processor, call that function. */
  if(mips_processor_type && !strcasecmp("tx79", mips_processor_type))
  {
    tx79_do_registers_info (regnum, fpregs);
    return;
  }
...
gdb_print_insn_mips (bfd_vma memaddr, disassemble_info *info)
{
  ...
  if(mips_processor_type && !strcasecmp("tx79", mips_processor_type))
    bfd_mach = bfd_mach_mips7900;
  else
    bfd_mach = TM_PRINT_INSN_MACH;
...
/* Store registers from a Cisco core file where `read_register' will
   find them.
   ...
   REG_ADDR is unused here.  */
void
fetch_cisco_core_registers (core_reg_sect, core_reg_size, which, reg_addr)
     char *core_reg_sect;
     ...
     CORE_ADDR reg_addr;
{
 if (GDB_TARGET_IS_MIPS64)
 {
  ...
  for (i = 0; i < 31; i++)
    {
      supply_register (i + 1, p + i * 8);
    }
  supply_register (HI_REGNUM, p + 248);
  supply_register (LO_REGNUM, p + 256);
  supply_register (PC_REGNUM, p + 264);

  /* Fudge for these 4 byte registers (supply_registers thinks the
     size is 8 - incorrectly).   */
  memset (buf, 0, 8);
  memcpy (buf + 4, p + 272, 4);
  supply_register (PS_REGNUM, buf);
  memcpy (buf + 4, p + 276, 4);
  supply_register (CAUSE_REGNUM, buf);
  memcpy (buf + 4, p + 280, 4);
  supply_register (BADVADDR_REGNUM, buf);
  supply_register (FP_REGNUM, &registers[REGISTER_BYTE (SP_REGNUM)]);
 } 
 else
 {
  typedef struct
    {
      unsigned char regs[64 * 4];       /* cpu registers */
      unsigned char sreg[4];            /* status register */
      unsigned char pad1[4];            /* padding for alignment */
      unsigned char mdlo_hi[4];
      unsigned char mdlo[4];            /* LO register */
      unsigned char mdhi_hi[4];
      unsigned char mdhi[4];            /* HI register */
      unsigned char badvaddr_hi[4];
      unsigned char badvaddr[4];        /* Bad Virtual address */
      unsigned char cause[4];           /* cause register */
      unsigned char pad2[4];            /* padding for alignment */
      unsigned char epc_hi[4];
      unsigned char epc[4];             /* epc register */
      unsigned char err_epc_hi[4];
      unsigned char err_epc[4];         /* error epc */
    }
  r4k_context_t;
  r4k_context_t *ctx;
  int i;

  ctx = (r4k_context_t *) core_reg_sect;
  memset (registers, 0, (size_t) REGISTER_BYTES);
  
  /* The corefile registers are in 64 bit format, even though the c4500
     runs in 32 bit mode.  We take only the low order 32 bits. */
  for (i = 0; i <= 62; i += 2)
    {
      supply_register ((i / 2), (char *) &ctx->regs[(i + 1) * 4]);
    }
  supply_register (PC_REGNUM, (char *) &ctx->epc);
  supply_register (PS_REGNUM, (char *) &ctx->sreg);
  supply_register (LO_REGNUM, (char *) &ctx->mdlo);
  supply_register (HI_REGNUM, (char *) &ctx->mdhi);
  supply_register (BADVADDR_REGNUM, (char *) &ctx->badvaddr);
  supply_register (CAUSE_REGNUM, (char *) &ctx->cause);
  supply_register (FP_REGNUM, &registers[REGISTER_BYTE (SP_REGNUM)]);
 }
}

struct type *
tx79_register_virtual_type(N)
     int N;
{
  return (((N) >= 0 && (N) < 32) ? builtin_type_int64
        : ((N) >= FP0_REGNUM && (N) < FP0_REGNUM+32) ? builtin_type_double
        : ((N) == 70 /* fsr */ || (N) == 71 /* fir */
           /* COP0 registers: */
           || ((N) >= 128  && (N) <= 149)) ? builtin_type_uint32
        : builtin_type_uint64);
}

struct type *
mips_register_virtual_type(N)
     int N;
{
  if(mips_processor_type && !strcasecmp("tx79", mips_processor_type))
    return((struct type *)tx79_register_virtual_type(N));
  else
    return (((N) >= FP0_REGNUM && (N) < FP0_REGNUM+32) ? builtin_type_float 
...
static void
tx79_128bit_printraw(char *buffer)
{
  int byte;
  if (TARGET_BYTE_ORDER == BFD_ENDIAN_BIG)
    for (byte = 0; byte < 8; byte++)
      printf_filtered ("%02x", (unsigned char) buffer[byte]);
  else
...
static void
tx79_128bit_onereg (regnum)
     int regnum;
{
  char raw_buffer[8];
  char *reg_name = REGISTER_NAME (regnum);
  if (regnum == 0)
    reg_name = "zr"; /* so that they're all 2 characters */
  printf_filtered(" %2s ", reg_name);
  if (read_relative_register_raw_bytes (regnum+90, raw_buffer))
    error ("can't read register %d (%s high)", regnum, REGISTER_NAME (regnum));
  tx79_128bit_printraw(raw_buffer);
  if (read_relative_register_raw_bytes (regnum, raw_buffer))
    error ("can't read register %d (%s low)", regnum, REGISTER_NAME (regnum));
  tx79_128bit_printraw(raw_buffer);
}

static int
tx79_128bit_register_row (regnum)
     int regnum;
{
  printf_filtered("R%-2d", regnum);
  tx79_128bit_onereg (regnum);
  tx79_128bit_onereg (regnum+1);
  printf_filtered("\n");
  return regnum+2;
}
  ...
  if (regnum != -1)             /* do one specified register */
    ...
    /* do all (or most) registers */
    {
      ...
      if (first_time)
        {
          first_time = 0;
          printf_filtered ("Note: R0 is really \"zero\", and the top 64 ");
          printf_filtered ("bits of (ex) reg \"rn\" is \"rnh\"\n");
        }
      regnum = 0;
      while (regnum < NUM_REGS)
        {
          if (TYPE_CODE (REGISTER_VIRTUAL_TYPE (regnum)) == TYPE_CODE_FLT)
            if (fpregs)         /* true for "INFO ALL-REGISTERS" command */
              ...
              regnum += MIPS_NUMREGS;   /* skip floating point regs */
          else if (regnum < 32) /* 128-bit GP registers */
            regnum = tx79_128bit_register_row (regnum);
          else if (regnum >= 90 && regnum < (90+32)) /* already printed */
            regnum = 90+32;
          else
            regnum = do_gp_register_row (regnum);       /* GP (int) regs */
        }
